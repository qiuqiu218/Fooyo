<<<<<<< HEAD
<?php
/*
Template Name:zh
*/
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta name="robots" content="noodp">
<link rel="canonical" href="http://www.fooyo.sg/zh/%e9%a6%96%e9%a1%b5/">
<meta property="og:locale" content="zh_CN">
<meta property="og:locale:alternate" content="en_US">
<meta property="og:type" content="website">
<meta property="og:title" content="首页 | Fooyo浮游科技|和浮游, 一起打造梦想的科技产品">
<meta property="og:url" content="http://www.fooyo.sg/zh/%e9%a6%96%e9%a1%b5/">
<meta property="og:site_name" content="Fooyo浮游科技|和浮游, 一起打造梦想的科技产品">
<meta name="twitter:card" content="summary">
<meta name="twitter:title" content="首页 | Fooyo浮游科技|和浮游, 一起打造梦想的科技产品">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/reset.css">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/common.css">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/style.css">
  <link rel="stylesheet" href="http://at.alicdn.com/t/font_jhj8p6w00wqxgvi.css">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/jquery.fullPage.css">
  <script src='<?php bloginfo('template_url'); ?>/js/rem.js'></script>
  <script src='<?php bloginfo('template_url'); ?>/js/jquery-3.1.0.min.js'></script>
  <script src='<?php bloginfo('template_url'); ?>/js/jquery.animate-colors-min.js'></script>
 <script src="<?php bloginfo('template_url'); ?>/js/jquery.fullPage.js"></script>
  <script src="<?php bloginfo('template_url'); ?>/js/common.js"></script>
  <link href="<?php bloginfo('template_url'); ?>/css/swiper-3.3.1.min.css" rel="stylesheet">
  <title>首页 | Fooyo浮游科技|和浮游, 一起打造梦想的科技产品</title>
</head>

<body>
 <div class="top">
    <h1>
      <svg width="133px" height="32px" viewBox="0 0 133 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
        <desc>Created with Sketch.</desc>
        <defs></defs>
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
          <g id="Fooyo_Logo" sketch:type="MSLayerGroup">
            <g id="patern_1_" sketch:type="MSShapeGroup">
              <g id="Ellipse_1" fill="#366695">
                <ellipse id="Oval" cx="6.03170254" cy="5.93015873" rx="6.03170254" ry="5.93015873"></ellipse>
              </g>
              <g id="Ellipse_1_copy_2" transform="translate(6.457926, 14.222222)" fill="#40ABB4">
              <ellipse id="Oval" cx="5.1146771" cy="5.05396825" rx="5.02426614" ry="4.93968254"></ellipse>
            </g>
          <g id="Ellipse_1_copy" transform="translate(16.532290, 2.920635)" fill="#47DCCB">
              <ellipse id="Oval" cx="4.5851272" cy="4.4952381" rx="4.52054795" ry="4.44444444"></ellipse>
          </g>
          <g id="Ellipse_1_copy_3" transform="translate(21.052838, 16.253968)" fill="#95F4E9">
              <ellipse id="Oval" cx="4.09432485" cy="4.01269841" rx="4.01682975" ry="3.94920635"></ellipse>
          </g>
        </g>
        <g id="word_1_" transform="translate(36.551859, 0.000000)" fill="#263248" sketch:type="MSShapeGroup">
          <g id="Ellipse_2_copy">
              <path d="M26.4258317,6.17142857 C20.7428571,6.17142857 16.9714286,9.76507937 16.9714286,15.3904762 C16.9714286,21.015873 20.9495108,24.3555556 26.1287671,24.3555556 C30.8688845,24.3555556 35.4669276,21.4349206 35.4669276,15.0603175 C35.4798434,9.84126984 31.8634051,6.17142857 26.4258317,6.17142857 L26.4258317,6.17142857 Z M26.2837573,20.4825397 C23.9330724,20.4825397 22.667319,18.2349206 22.667319,15.2507937 C22.667319,12.6857143 23.6489237,10.0190476 26.2837573,10.0190476 C28.7765166,10.0190476 29.7968689,12.647619 29.7968689,15.2507937 C29.7968689,18.3746032 28.4277886,20.4825397 26.2837573,20.4825397 L26.2837573,20.4825397 Z M68.1052838,15.0984127 C67.8469667,16.0888889 67.562818,17.4095238 67.304501,18.3365079 L67.2011742,18.3365079 C66.9816047,17.4095238 66.6199609,16.1269841 66.3358121,15.0984127 L63.6880626,6.55238095 L57.5659491,6.55238095 L64.0497065,22.4761905 C64.2692759,22.9079365 64.3080235,23.1873016 64.3080235,23.3650794 C64.3080235,23.6190476 64.2046967,23.8984127 64.0238748,24.152381 C63.481409,25.0412698 62.5772994,25.8920635 61.7377691,26.3238095 C60.9111546,26.8190476 60.0328767,27.1365079 59.3483366,27.2507937 L59.3612524,27.2761905 C58.353816,27.5555556 57.6176125,28.4571429 57.6176125,29.5365079 C57.6176125,30.831746 58.6896282,31.8857143 60.007045,31.8857143 C60.1749511,31.8857143 60.3428571,31.8603175 60.5107632,31.8349206 L60.5107632,31.847619 C61.8927593,31.6698413 63.9205479,31.0984127 65.8320939,29.4603175 C67.821135,27.7206349 69.5260274,24.9396825 72.0575342,18.031746 L76.1906067,6.57777778 L70.2880626,6.57777778 L68.1052838,15.0984127 L68.1052838,15.0984127 Z M47.4270059,6.17142857 C41.7440313,6.17142857 37.9726027,9.76507937 37.9726027,15.3904762 C37.9726027,21.015873 41.9506849,24.3555556 47.1299413,24.3555556 C51.8700587,24.3555556 56.4681018,21.4349206 56.4681018,15.0603175 C56.4810176,9.84126984 52.8645793,6.17142857 47.4270059,6.17142857 L47.4270059,6.17142857 Z M47.2849315,20.4825397 C44.9342466,20.4825397 43.6684932,18.2349206 43.6684932,15.2507937 C43.6684932,12.6857143 44.6500978,10.0190476 47.2849315,10.0190476 C49.7776908,10.0190476 50.7980431,12.647619 50.7980431,15.2507937 C50.7980431,18.3746032 49.416047,20.4825397 47.2849315,20.4825397 L47.2849315,20.4825397 Z M14.6982387,4.43174603 C14.7886497,4.44444444 14.8919765,4.45714286 14.9823875,4.45714286 C16.2610568,4.45714286 17.3072407,3.42857143 17.3072407,2.17142857 C17.3072407,1.16825397 16.6485323,0.317460317 15.7444227,0.0126984127 L0.0387475538,0.0126984127 L0.0387475538,23.9746032 L0.142074364,23.9746032 C0.193737769,25.4095238 1.38199609,26.552381 2.85440313,26.552381 C4.32681018,26.552381 5.51506849,25.4095238 5.5667319,23.9746032 L5.5925636,23.9746032 L5.5925636,14.3238095 L13.807045,14.3238095 C13.9232877,14.3365079 14.0395303,14.3492063 14.155773,14.3492063 C15.4086106,14.3492063 16.416047,13.3587302 16.416047,12.1269841 C16.416047,10.9587302 15.5119374,10.0063492 14.3495108,9.91746032 L14.3495108,9.9047619 L14.1816047,9.9047619 L14.155773,9.9047619 L14.1299413,9.9047619 L5.5925636,9.9047619 L5.5925636,4.43174603 L14.6982387,4.43174603 L14.6982387,4.43174603 Z M86.3941292,6.17142857 C80.7111546,6.17142857 76.939726,9.76507937 76.939726,15.3904762 C76.939726,21.015873 80.9178082,24.3555556 86.0970646,24.3555556 C90.837182,24.3555556 95.435225,21.4349206 95.435225,15.0603175 C95.4481409,9.84126984 91.8187867,6.17142857 86.3941292,6.17142857 L86.3941292,6.17142857 Z M86.2391389,20.4825397 C83.888454,20.4825397 82.6227006,18.2349206 82.6227006,15.2507937 C82.6227006,12.6857143 83.6043053,10.0190476 86.2391389,10.0190476 C88.7318982,10.0190476 89.7522505,12.647619 89.7522505,15.2507937 C89.7522505,18.3746032 88.3831703,20.4825397 86.2391389,20.4825397 L86.2391389,20.4825397 Z" id="Shape">
            </path>
          </g>
        </g>
      </svg>
    </h1>
<?php get_header();?>
  </div>
  <script>
  $(".lan").append($('.lang-item>a'))
    $(".lang-item").remove()


  </script>
   <div id="fullpage">
    <div class="page page1 current section" id="section0">
        <div class="contain">
            <section id='one' >
              <div class="f"></div>
              <div class="f"></div>
              <div class="f"></div>
              <div class="f"></div>
              <div class="triangle">
                <canvas id="c1" width="420" height="622"></canvas>
              </div>
              <h2>我们<span></span><em></em></h2>
              <a href="javascript:;" class="aircraft"></a>
            </section>
        </div>
    </div>
    <div class="page page2 section" id="section1">
        <div class="contain">
            <section id='two' >
            <canvas id="pixie" style="position: absolute;left: 0;top: 0;"></canvas>
              <div class="h2">
                <div>
                  <p>浮</p>
                  <p>游</p>
                </div>
                <small>fooyo</small>
                      <?php 
$category_id = get_cat_ID( '关于'); 
$category_link = get_category_link( $category_id ); 
?> 
              </div>
              <p class="content">我们是一个科技咨询公司。</p>
              <p class="content">我们帮助大公司在移动科技领域创新,我们也和初创公司一起创建新产品。</p>
              <p class="content">我们具备 iOS 应用、安卓应用以及网站整套方案的开发能力。</p>
              <p class="content">我们精益求精，备受客户青睐。</p>
              <a class="link" href="<?php echo $category_link;?>" target="_blank">了解更多>></a>
            </section>
        </div>
    </div>
  <div class="page page3 section" id="section2">
        <div class="contain">
            <section id='three'>
              <div class="swiper-container">
                <div class="swiper-wrapper">
                  <div class="swiper-slide">
                    <div class="earth">
                      <img src="<?php bloginfo('template_url'); ?>/images/air.gif" alt="新加坡移动应用开发公司">
                    </div>
                    <div class="h2">
                      <p>全球性服务</p>
                      <div>
                        <p>我们和全球的创新企业和创新者合作,</p>
                        <p>推出<em>移动创新产品</em>,让人们的生活更美好。</p>
                      </div>
                      <div>
                        <p>浮游的业务涵盖全球。</p>
                        <p>只要您有需求,地域不是问题,沟通不是问题,做出满意的新产品才是关键。</p>
                      </div>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="h2">
                      <p>专业性服务</p>
                      <small>从零到一</small>
                      <div>
                        <p>我们非常享受与客户和合作伙伴一起从零开始打造产品。我们会因为产品被千千万万的用户喜欢而感到兴奋。</p>
                        <p>我们相信好产品是通过不断迭代雕琢出来的。我们用敏捷开发的方式来指导创新的过程,</p>
                        <p>从点子到设计,到开发,都贯彻始终。</p>
                      </div>
                    </div>
                    <ul>
                      <li>
                        <div></div>
                        <p>点子</p>
                      </li>
                      <li>
                        <div></div>
                        <p>需求分析/原型图</p>
                      </li>
                      <li>
                        <div></div>
                        <p>视觉设计/交互设计</p>
                      </li>
                      <li>
                        <div></div>
                        <p>开发</p>
                      </li>
                      <li>
                        <div></div>
                        <p>新产品</p>
                      </li>
                      <li>
                        <div></div>
                        <p>测试/优化</p>
                      </li>
                      <li>
                        <div></div>
                        <p>上线</p>
                      </li>
                    </ul>
                  </div>
                  <div class="swiper-slide">
                    <div class="step">
                      <div class="advisory step-item">
                        <span class="number">1</span>
                        <small></small>
                        <em>咨询</em>
                      </div>
                      <div class="design step-item">
                        <span class="number">2</span>
                        <small></small>
                        <em>设计</em>
                      </div>
                      <div class="agile step-item">
                        <span class="number">3</span>
                        <small></small>
                        <em>敏捷开发</em>
                      </div>
                    </div>
                    <div class="content">
                      <h2>高效性服务</h2>
                      <small>我们的产品开发的过程采用<em>敏捷开发</em>的模式。</small>
                      <p>项目一般会被拆分成2-3周的小周期。各个周期我们都会提供一个新的版本做用户测试。</p>
                     <p>我们快而精美，精益求精，识变化而知应对。</p>
                    </div>
                  </div>
                </div>
                <div class="swiper-button-prev"></div>
                <div class="swiper-button-next"></div>
              </div>
            </section>
        </div>
    </div>
                          <?php 
$category_id = get_cat_ID( '案例'); 
$category_link = get_category_link( $category_id ); 
?> 
          <div class="page page4 section" id="section3">
          <div class="contain">
              <section id="four" >
              <div class="ct">
                <div class="c1">
                  <a href="<?php echo $category_link."?goal=餐饮"; ?> "class="mask">
                    <span>Food & Beverage</span>
                    <small>[餐饮]</small>
                    <p>俗话说得好，民以食为天。随着智能手机的普及，餐饮行业早已掀起了一场线上线下浑然天成的维新革命。Fooyo抱着对美食的推崇和对技术的严谨，在过去的一年中有幸和几个餐饮类创业者有过深入合作。</p>
                  </a>
                </div>
                <div>
                  <div class="c2">
                    <a href="<?php echo $category_link."?goal=医疗服务"; ?>" class="mask">
                        <span>Healthcare</span>
                        <small>[医疗服务]</small>
                        <p>科技正在改变我们的生活，随着各种智能产品和服务的推出，健康领域也在不断出现创新革命。</p>
                    </a>
                  </div>
                  <div>
                    <div class="c3">
                    <a href="<?php echo $category_link."?goal=旅行"; ?>" class="mask">
                      <span>Tourism and Hospitality</span>
                      <small>[旅行和住宿]</small>
                      <p>旅行和住宿行业已迈入移动互联网时代，现在旅行已经不仅仅是出行前的订票，订酒店，移动互联网给“出行时”带来了很多想象的空间。</p>
                    </a>
                    </div>
                    <div class="c4">
                      <a href="<?php echo $category_link."?goal=教育科技"; ?>" class="mask">
                        <span>Edutech</span>
                        <small>[教育科技]</small>
                        <p>
  教育科技涵盖很多层面，从很早的教案课件管理，到在线网络视频教学，app自学，甚至到未来的AR/VR学校，个性化AI教育，教育和科技的结合真是有数不尽的可能性。
                        </p>
                      </a>
                    </div>
                  </div>
                </div>
                <div>
                  <div class="c5">
                    <a href="<?php echo $category_link."?goal=物联网"; ?>" class="mask">
                      <span>IOT</span>
                      <small>[物联网]</small>
                      <p>物联网已经在发生，从扎克伯格的家庭智能系统，到物流，农业，医疗数据，物联网正在不断地渗透着我们的生活，让生活生产变得更加智能化，数据化。</p>
                    </a>
                  </div>
                  <div class="other">
                    <div class="mask">
                      <span>Other</span>
                      <small>[其他]</small>
                      <a href="<?php echo $category_link."?goal=其他"; ?>" class="link">点击进入</a>
                    </div>
                  </div>
                </div>
              </div>
            </section>
        </div>
    </div>
    <div class="page page5 section" id="section4">
        <div class="contain">
            <section id="five" >
              <img src="<?php bloginfo('template_url'); ?>/images/footer-06-01.png" alt="北京IT咨询公司" class="dec">
              <a href="http://42.96.189.31/?cat=34&lang=zh" class="joinus">加入我们</a>
              <p class="share">分享</p>
              <div class="icon">
                <a href="#" class="iconfont icon-Facebook"></a>
                <a href="#" class="iconfont icon-Twitter"></a>
                <a href="#" class="iconfont icon-Google"></a>
              </div>
              <footer>
                <svg width="133px" height="32px" viewBox="0 0 133 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
                  <desc>Created with Sketch.</desc>
                  <defs></defs>
                  <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
                    <g id="Fooyo_Logo" sketch:type="MSLayerGroup">
                      <g id="patern_1_" sketch:type="MSShapeGroup">
                        <g id="Ellipse_1" fill="#366695">
                          <ellipse id="Oval" cx="6.03170254" cy="5.93015873" rx="6.03170254" ry="5.93015873"></ellipse>
                        </g>
                        <g id="Ellipse_1_copy_2" transform="translate(6.457926, 14.222222)" fill="#40ABB4">
                        <ellipse id="Oval" cx="5.1146771" cy="5.05396825" rx="5.02426614" ry="4.93968254"></ellipse>
                      </g>
                    <g id="Ellipse_1_copy" transform="translate(16.532290, 2.920635)" fill="#47DCCB">
                        <ellipse id="Oval" cx="4.5851272" cy="4.4952381" rx="4.52054795" ry="4.44444444"></ellipse>
                    </g>
                    <g id="Ellipse_1_copy_3" transform="translate(21.052838, 16.253968)" fill="#95F4E9">
                        <ellipse id="Oval" cx="4.09432485" cy="4.01269841" rx="4.01682975" ry="3.94920635"></ellipse>
                    </g>
                  </g>
                  <g id="word_1_" transform="translate(36.551859, 0.000000)" fill="#263248" sketch:type="MSShapeGroup">
                    <g id="Ellipse_2_copy">
                        <path d="M26.4258317,6.17142857 C20.7428571,6.17142857 16.9714286,9.76507937 16.9714286,15.3904762 C16.9714286,21.015873 20.9495108,24.3555556 26.1287671,24.3555556 C30.8688845,24.3555556 35.4669276,21.4349206 35.4669276,15.0603175 C35.4798434,9.84126984 31.8634051,6.17142857 26.4258317,6.17142857 L26.4258317,6.17142857 Z M26.2837573,20.4825397 C23.9330724,20.4825397 22.667319,18.2349206 22.667319,15.2507937 C22.667319,12.6857143 23.6489237,10.0190476 26.2837573,10.0190476 C28.7765166,10.0190476 29.7968689,12.647619 29.7968689,15.2507937 C29.7968689,18.3746032 28.4277886,20.4825397 26.2837573,20.4825397 L26.2837573,20.4825397 Z M68.1052838,15.0984127 C67.8469667,16.0888889 67.562818,17.4095238 67.304501,18.3365079 L67.2011742,18.3365079 C66.9816047,17.4095238 66.6199609,16.1269841 66.3358121,15.0984127 L63.6880626,6.55238095 L57.5659491,6.55238095 L64.0497065,22.4761905 C64.2692759,22.9079365 64.3080235,23.1873016 64.3080235,23.3650794 C64.3080235,23.6190476 64.2046967,23.8984127 64.0238748,24.152381 C63.481409,25.0412698 62.5772994,25.8920635 61.7377691,26.3238095 C60.9111546,26.8190476 60.0328767,27.1365079 59.3483366,27.2507937 L59.3612524,27.2761905 C58.353816,27.5555556 57.6176125,28.4571429 57.6176125,29.5365079 C57.6176125,30.831746 58.6896282,31.8857143 60.007045,31.8857143 C60.1749511,31.8857143 60.3428571,31.8603175 60.5107632,31.8349206 L60.5107632,31.847619 C61.8927593,31.6698413 63.9205479,31.0984127 65.8320939,29.4603175 C67.821135,27.7206349 69.5260274,24.9396825 72.0575342,18.031746 L76.1906067,6.57777778 L70.2880626,6.57777778 L68.1052838,15.0984127 L68.1052838,15.0984127 Z M47.4270059,6.17142857 C41.7440313,6.17142857 37.9726027,9.76507937 37.9726027,15.3904762 C37.9726027,21.015873 41.9506849,24.3555556 47.1299413,24.3555556 C51.8700587,24.3555556 56.4681018,21.4349206 56.4681018,15.0603175 C56.4810176,9.84126984 52.8645793,6.17142857 47.4270059,6.17142857 L47.4270059,6.17142857 Z M47.2849315,20.4825397 C44.9342466,20.4825397 43.6684932,18.2349206 43.6684932,15.2507937 C43.6684932,12.6857143 44.6500978,10.0190476 47.2849315,10.0190476 C49.7776908,10.0190476 50.7980431,12.647619 50.7980431,15.2507937 C50.7980431,18.3746032 49.416047,20.4825397 47.2849315,20.4825397 L47.2849315,20.4825397 Z M14.6982387,4.43174603 C14.7886497,4.44444444 14.8919765,4.45714286 14.9823875,4.45714286 C16.2610568,4.45714286 17.3072407,3.42857143 17.3072407,2.17142857 C17.3072407,1.16825397 16.6485323,0.317460317 15.7444227,0.0126984127 L0.0387475538,0.0126984127 L0.0387475538,23.9746032 L0.142074364,23.9746032 C0.193737769,25.4095238 1.38199609,26.552381 2.85440313,26.552381 C4.32681018,26.552381 5.51506849,25.4095238 5.5667319,23.9746032 L5.5925636,23.9746032 L5.5925636,14.3238095 L13.807045,14.3238095 C13.9232877,14.3365079 14.0395303,14.3492063 14.155773,14.3492063 C15.4086106,14.3492063 16.416047,13.3587302 16.416047,12.1269841 C16.416047,10.9587302 15.5119374,10.0063492 14.3495108,9.91746032 L14.3495108,9.9047619 L14.1816047,9.9047619 L14.155773,9.9047619 L14.1299413,9.9047619 L5.5925636,9.9047619 L5.5925636,4.43174603 L14.6982387,4.43174603 L14.6982387,4.43174603 Z M86.3941292,6.17142857 C80.7111546,6.17142857 76.939726,9.76507937 76.939726,15.3904762 C76.939726,21.015873 80.9178082,24.3555556 86.0970646,24.3555556 C90.837182,24.3555556 95.435225,21.4349206 95.435225,15.0603175 C95.4481409,9.84126984 91.8187867,6.17142857 86.3941292,6.17142857 L86.3941292,6.17142857 Z M86.2391389,20.4825397 C83.888454,20.4825397 82.6227006,18.2349206 82.6227006,15.2507937 C82.6227006,12.6857143 83.6043053,10.0190476 86.2391389,10.0190476 C88.7318982,10.0190476 89.7522505,12.647619 89.7522505,15.2507937 C89.7522505,18.3746032 88.3831703,20.4825397 86.2391389,20.4825397 L86.2391389,20.4825397 Z" id="Shape">
                      </path>
                    </g>
                  </g>
                </svg>
                <p>
                  <span>地址：3 Science Park Drive,The Franklin #02-07,Singapore</span>
                  <span>邮编：118223</span>
                  <span>手机号：(+65)84127233</span>
                  <span>电子邮箱：hi@fooyo.sg</span>
                </p>
              </footer>
            </section>
        </div>
    </div>
  </div>
  <ul id="navBar">
  <li data-menuanchor="firstPage"><a href="#firstPage"></a></li>
    <li data-menuanchor="secondPage"><a href="#secondPage"></a></li>
    <li data-menuanchor="3rdPage"><a href="#3rdPage"></a></li>
    <li data-menuanchor="4thpage"><a href="#4thpage"></a></li>
    <li data-menuanchor="lastPage"><a href="#lastPage"></a></li>
  </ul>
  <script src="<?php bloginfo('template_url'); ?>/js/swiper-3.3.1.min.js"></script>
  <script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/canvasRender.js"></script>
  <script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/fooyo.js"></script>
  <script src='<?php bloginfo('template_url'); ?>/js/app.js'></script>
  <script>
    $(".current-menu-item").addClass('active')

  </script>

</body>
</html>
=======
<?php
/*
Template Name:zh
*/
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta name="robots" content="noodp">
<link rel="canonical" href="http://www.fooyo.sg/zh/%e9%a6%96%e9%a1%b5/">
<meta property="og:locale" content="zh_CN">
<meta property="og:locale:alternate" content="en_US">
<meta property="og:type" content="website">
<meta property="og:title" content="首页 | Fooyo浮游科技|和浮游, 一起打造梦想的科技产品">
<meta property="og:url" content="http://www.fooyo.sg/zh/%e9%a6%96%e9%a1%b5/">
<meta property="og:site_name" content="Fooyo浮游科技|和浮游, 一起打造梦想的科技产品">
<meta name="twitter:card" content="summary">
<meta name="twitter:title" content="首页 | Fooyo浮游科技|和浮游, 一起打造梦想的科技产品">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/reset.css">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/common.css">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/style.css">
  <link rel="stylesheet" href="http://at.alicdn.com/t/font_jhj8p6w00wqxgvi.css">
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/jquery.fullPage.css">
  <script src='<?php bloginfo('template_url'); ?>/js/rem.js'></script>
  <script src='<?php bloginfo('template_url'); ?>/js/jquery-3.1.0.min.js'></script>
  <script src='<?php bloginfo('template_url'); ?>/js/jquery.animate-colors-min.js'></script>
 <script src="<?php bloginfo('template_url'); ?>/js/jquery.fullPage.js"></script>
  <script src="<?php bloginfo('template_url'); ?>/js/common.js"></script>
  <link href="<?php bloginfo('template_url'); ?>/css/swiper-3.3.1.min.css" rel="stylesheet">
  <title>首页 | Fooyo浮游科技|和浮游, 一起打造梦想的科技产品</title>
</head>

<body>
 <div class="top">
    <h1>
      <svg width="133px" height="32px" viewBox="0 0 133 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
        <desc>Created with Sketch.</desc>
        <defs></defs>
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
          <g id="Fooyo_Logo" sketch:type="MSLayerGroup">
            <g id="patern_1_" sketch:type="MSShapeGroup">
              <g id="Ellipse_1" fill="#366695">
                <ellipse id="Oval" cx="6.03170254" cy="5.93015873" rx="6.03170254" ry="5.93015873"></ellipse>
              </g>
              <g id="Ellipse_1_copy_2" transform="translate(6.457926, 14.222222)" fill="#40ABB4">
              <ellipse id="Oval" cx="5.1146771" cy="5.05396825" rx="5.02426614" ry="4.93968254"></ellipse>
            </g>
          <g id="Ellipse_1_copy" transform="translate(16.532290, 2.920635)" fill="#47DCCB">
              <ellipse id="Oval" cx="4.5851272" cy="4.4952381" rx="4.52054795" ry="4.44444444"></ellipse>
          </g>
          <g id="Ellipse_1_copy_3" transform="translate(21.052838, 16.253968)" fill="#95F4E9">
              <ellipse id="Oval" cx="4.09432485" cy="4.01269841" rx="4.01682975" ry="3.94920635"></ellipse>
          </g>
        </g>
        <g id="word_1_" transform="translate(36.551859, 0.000000)" fill="#263248" sketch:type="MSShapeGroup">
          <g id="Ellipse_2_copy">
              <path d="M26.4258317,6.17142857 C20.7428571,6.17142857 16.9714286,9.76507937 16.9714286,15.3904762 C16.9714286,21.015873 20.9495108,24.3555556 26.1287671,24.3555556 C30.8688845,24.3555556 35.4669276,21.4349206 35.4669276,15.0603175 C35.4798434,9.84126984 31.8634051,6.17142857 26.4258317,6.17142857 L26.4258317,6.17142857 Z M26.2837573,20.4825397 C23.9330724,20.4825397 22.667319,18.2349206 22.667319,15.2507937 C22.667319,12.6857143 23.6489237,10.0190476 26.2837573,10.0190476 C28.7765166,10.0190476 29.7968689,12.647619 29.7968689,15.2507937 C29.7968689,18.3746032 28.4277886,20.4825397 26.2837573,20.4825397 L26.2837573,20.4825397 Z M68.1052838,15.0984127 C67.8469667,16.0888889 67.562818,17.4095238 67.304501,18.3365079 L67.2011742,18.3365079 C66.9816047,17.4095238 66.6199609,16.1269841 66.3358121,15.0984127 L63.6880626,6.55238095 L57.5659491,6.55238095 L64.0497065,22.4761905 C64.2692759,22.9079365 64.3080235,23.1873016 64.3080235,23.3650794 C64.3080235,23.6190476 64.2046967,23.8984127 64.0238748,24.152381 C63.481409,25.0412698 62.5772994,25.8920635 61.7377691,26.3238095 C60.9111546,26.8190476 60.0328767,27.1365079 59.3483366,27.2507937 L59.3612524,27.2761905 C58.353816,27.5555556 57.6176125,28.4571429 57.6176125,29.5365079 C57.6176125,30.831746 58.6896282,31.8857143 60.007045,31.8857143 C60.1749511,31.8857143 60.3428571,31.8603175 60.5107632,31.8349206 L60.5107632,31.847619 C61.8927593,31.6698413 63.9205479,31.0984127 65.8320939,29.4603175 C67.821135,27.7206349 69.5260274,24.9396825 72.0575342,18.031746 L76.1906067,6.57777778 L70.2880626,6.57777778 L68.1052838,15.0984127 L68.1052838,15.0984127 Z M47.4270059,6.17142857 C41.7440313,6.17142857 37.9726027,9.76507937 37.9726027,15.3904762 C37.9726027,21.015873 41.9506849,24.3555556 47.1299413,24.3555556 C51.8700587,24.3555556 56.4681018,21.4349206 56.4681018,15.0603175 C56.4810176,9.84126984 52.8645793,6.17142857 47.4270059,6.17142857 L47.4270059,6.17142857 Z M47.2849315,20.4825397 C44.9342466,20.4825397 43.6684932,18.2349206 43.6684932,15.2507937 C43.6684932,12.6857143 44.6500978,10.0190476 47.2849315,10.0190476 C49.7776908,10.0190476 50.7980431,12.647619 50.7980431,15.2507937 C50.7980431,18.3746032 49.416047,20.4825397 47.2849315,20.4825397 L47.2849315,20.4825397 Z M14.6982387,4.43174603 C14.7886497,4.44444444 14.8919765,4.45714286 14.9823875,4.45714286 C16.2610568,4.45714286 17.3072407,3.42857143 17.3072407,2.17142857 C17.3072407,1.16825397 16.6485323,0.317460317 15.7444227,0.0126984127 L0.0387475538,0.0126984127 L0.0387475538,23.9746032 L0.142074364,23.9746032 C0.193737769,25.4095238 1.38199609,26.552381 2.85440313,26.552381 C4.32681018,26.552381 5.51506849,25.4095238 5.5667319,23.9746032 L5.5925636,23.9746032 L5.5925636,14.3238095 L13.807045,14.3238095 C13.9232877,14.3365079 14.0395303,14.3492063 14.155773,14.3492063 C15.4086106,14.3492063 16.416047,13.3587302 16.416047,12.1269841 C16.416047,10.9587302 15.5119374,10.0063492 14.3495108,9.91746032 L14.3495108,9.9047619 L14.1816047,9.9047619 L14.155773,9.9047619 L14.1299413,9.9047619 L5.5925636,9.9047619 L5.5925636,4.43174603 L14.6982387,4.43174603 L14.6982387,4.43174603 Z M86.3941292,6.17142857 C80.7111546,6.17142857 76.939726,9.76507937 76.939726,15.3904762 C76.939726,21.015873 80.9178082,24.3555556 86.0970646,24.3555556 C90.837182,24.3555556 95.435225,21.4349206 95.435225,15.0603175 C95.4481409,9.84126984 91.8187867,6.17142857 86.3941292,6.17142857 L86.3941292,6.17142857 Z M86.2391389,20.4825397 C83.888454,20.4825397 82.6227006,18.2349206 82.6227006,15.2507937 C82.6227006,12.6857143 83.6043053,10.0190476 86.2391389,10.0190476 C88.7318982,10.0190476 89.7522505,12.647619 89.7522505,15.2507937 C89.7522505,18.3746032 88.3831703,20.4825397 86.2391389,20.4825397 L86.2391389,20.4825397 Z" id="Shape">
            </path>
          </g>
        </g>
      </svg>
    </h1>
<?php get_header();?>
  </div>
  <script>
  $(".lan").append($('.lang-item>a'))
    $(".lang-item").remove()


  </script>
   <div id="fullpage">
    <div class="page page1 current section" id="section0">
        <div class="contain">
            <section id='one' >
              <div class="f"></div>
              <div class="f"></div>
              <div class="f"></div>
              <div class="f"></div>
              <div class="triangle">
                <canvas id="c1" width="420" height="622"></canvas>
              </div>
              <h2>我们<span></span><em></em></h2>
              <a href="javascript:;" class="aircraft"></a>
            </section>
        </div>
    </div>
    <div class="page page2 section" id="section1">
        <div class="contain">
            <section id='two' >
            <canvas id="pixie" style="position: absolute;left: 0;top: 0;"></canvas>
              <div class="h2">
                <div>
                  <p>浮</p>
                  <p>游</p>
                </div>
                <small>fooyo</small>
                      <?php 
$category_id = get_cat_ID( '关于'); 
$category_link = get_category_link( $category_id ); 
?> 
              </div>
              <p class="content">我们是一个科技咨询公司。</p>
              <p class="content">我们帮助大公司在移动科技领域创新,我们也和初创公司一起创建新产品。</p>
              <p class="content">我们具备 iOS 应用、安卓应用以及网站整套方案的开发能力。</p>
              <p class="content">我们精益求精，备受客户青睐。</p>
              <a class="link" href="<?php echo $category_link;?>" target="_blank">了解更多>></a>
            </section>
        </div>
    </div>
  <div class="page page3 section" id="section2">
        <div class="contain">
            <section id='three'>
              <div class="swiper-container">
                <div class="swiper-wrapper">
                  <div class="swiper-slide">
                    <div class="earth">
                      <img src="<?php bloginfo('template_url'); ?>/images/air.gif" alt="新加坡移动应用开发公司">
                    </div>
                    <div class="h2">
                      <p>全球性服务</p>
                      <div>
                        <p>我们和全球的创新企业和创新者合作,</p>
                        <p>推出<em>移动创新产品</em>,让人们的生活更美好。</p>
                      </div>
                      <div>
                        <p>浮游的业务涵盖全球。</p>
                        <p>只要您有需求,地域不是问题,沟通不是问题,做出满意的新产品才是关键。</p>
                      </div>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="h2">
                      <p>专业性服务</p>
                      <small>从零到一</small>
                      <div>
                        <p>我们非常享受与客户和合作伙伴一起从零开始打造产品。我们会因为产品被千千万万的用户喜欢而感到兴奋。</p>
                        <p>我们相信好产品是通过不断迭代雕琢出来的。我们用敏捷开发的方式来指导创新的过程,</p>
                        <p>从点子到设计,到开发,都贯彻始终。</p>
                      </div>
                    </div>
                    <ul>
                      <li>
                        <div></div>
                        <p>点子</p>
                      </li>
                      <li>
                        <div></div>
                        <p>需求分析/原型图</p>
                      </li>
                      <li>
                        <div></div>
                        <p>视觉设计/交互设计</p>
                      </li>
                      <li>
                        <div></div>
                        <p>开发</p>
                      </li>
                      <li>
                        <div></div>
                        <p>新产品</p>
                      </li>
                      <li>
                        <div></div>
                        <p>测试/优化</p>
                      </li>
                      <li>
                        <div></div>
                        <p>上线</p>
                      </li>
                    </ul>
                  </div>
                  <div class="swiper-slide">
                    <div class="step">
                      <div class="advisory step-item">
                        <span class="number">1</span>
                        <small></small>
                        <em>咨询</em>
                      </div>
                      <div class="design step-item">
                        <span class="number">2</span>
                        <small></small>
                        <em>设计</em>
                      </div>
                      <div class="agile step-item">
                        <span class="number">3</span>
                        <small></small>
                        <em>敏捷开发</em>
                      </div>
                    </div>
                    <div class="content">
                      <h2>高效性服务</h2>
                      <small>我们的产品开发的过程采用<em>敏捷开发</em>的模式。</small>
                      <p>项目一般会被拆分成2-3周的小周期。各个周期我们都会提供一个新的版本做用户测试。</p>
                     <p>我们快而精美，精益求精，识变化而知应对。</p>
                    </div>
                  </div>
                </div>
                <div class="swiper-button-prev"></div>
                <div class="swiper-button-next"></div>
              </div>
            </section>
        </div>
    </div>
                          <?php 
$category_id = get_cat_ID( '案例'); 
$category_link = get_category_link( $category_id ); 
?> 
          <div class="page page4 section" id="section3">
          <div class="contain">
              <section id="four" >
              <div class="ct">
                <div class="c1">
                  <a href="<?php echo $category_link."?goal=餐饮"; ?> "class="mask">
                    <span>Food & Beverage</span>
                    <small>[餐饮]</small>
                    <p>俗话说得好，民以食为天。随着智能手机的普及，餐饮行业早已掀起了一场线上线下浑然天成的维新革命。Fooyo抱着对美食的推崇和对技术的严谨，在过去的一年中有幸和几个餐饮类创业者有过深入合作。</p>
                  </a>
                </div>
                <div>
                  <div class="c2">
                    <a href="<?php echo $category_link."?goal=医疗服务"; ?>" class="mask">
                        <span>Healthcare</span>
                        <small>[医疗服务]</small>
                        <p>科技正在改变我们的生活，随着各种智能产品和服务的推出，健康领域也在不断出现创新革命。</p>
                    </a>
                  </div>
                  <div>
                    <div class="c3">
                    <a href="<?php echo $category_link."?goal=旅行"; ?>" class="mask">
                      <span>Tourism and Hospitality</span>
                      <small>[旅行和住宿]</small>
                      <p>旅行和住宿行业已迈入移动互联网时代，现在旅行已经不仅仅是出行前的订票，订酒店，移动互联网给“出行时”带来了很多想象的空间。</p>
                    </a>
                    </div>
                    <div class="c4">
                      <a href="<?php echo $category_link."?goal=教育科技"; ?>" class="mask">
                        <span>Edutech</span>
                        <small>[教育科技]</small>
                        <p>
  教育科技涵盖很多层面，从很早的教案课件管理，到在线网络视频教学，app自学，甚至到未来的AR/VR学校，个性化AI教育，教育和科技的结合真是有数不尽的可能性。
                        </p>
                      </a>
                    </div>
                  </div>
                </div>
                <div>
                  <div class="c5">
                    <a href="<?php echo $category_link."?goal=物联网"; ?>" class="mask">
                      <span>IOT</span>
                      <small>[物联网]</small>
                      <p>物联网已经在发生，从扎克伯格的家庭智能系统，到物流，农业，医疗数据，物联网正在不断地渗透着我们的生活，让生活生产变得更加智能化，数据化。</p>
                    </a>
                  </div>
                  <div class="other">
                    <div class="mask">
                      <span>Other</span>
                      <small>[其他]</small>
                      <a href="<?php echo $category_link."?goal=其他"; ?>" class="link">点击进入</a>
                    </div>
                  </div>
                </div>
              </div>
            </section>
        </div>
    </div>
    <div class="page page5 section" id="section4">
        <div class="contain">
            <section id="five" >
              <img src="<?php bloginfo('template_url'); ?>/images/footer-06-01.png" alt="北京IT咨询公司" class="dec">
              <a href="http://42.96.189.31/?cat=34&lang=zh" class="joinus">加入我们</a>
              <p class="share">分享</p>
              <div class="icon">
                <a href="#" class="iconfont icon-Facebook"></a>
                <a href="#" class="iconfont icon-Twitter"></a>
                <a href="#" class="iconfont icon-Google"></a>
              </div>
              <footer>
                <svg width="133px" height="32px" viewBox="0 0 133 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
                  <desc>Created with Sketch.</desc>
                  <defs></defs>
                  <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
                    <g id="Fooyo_Logo" sketch:type="MSLayerGroup">
                      <g id="patern_1_" sketch:type="MSShapeGroup">
                        <g id="Ellipse_1" fill="#366695">
                          <ellipse id="Oval" cx="6.03170254" cy="5.93015873" rx="6.03170254" ry="5.93015873"></ellipse>
                        </g>
                        <g id="Ellipse_1_copy_2" transform="translate(6.457926, 14.222222)" fill="#40ABB4">
                        <ellipse id="Oval" cx="5.1146771" cy="5.05396825" rx="5.02426614" ry="4.93968254"></ellipse>
                      </g>
                    <g id="Ellipse_1_copy" transform="translate(16.532290, 2.920635)" fill="#47DCCB">
                        <ellipse id="Oval" cx="4.5851272" cy="4.4952381" rx="4.52054795" ry="4.44444444"></ellipse>
                    </g>
                    <g id="Ellipse_1_copy_3" transform="translate(21.052838, 16.253968)" fill="#95F4E9">
                        <ellipse id="Oval" cx="4.09432485" cy="4.01269841" rx="4.01682975" ry="3.94920635"></ellipse>
                    </g>
                  </g>
                  <g id="word_1_" transform="translate(36.551859, 0.000000)" fill="#263248" sketch:type="MSShapeGroup">
                    <g id="Ellipse_2_copy">
                        <path d="M26.4258317,6.17142857 C20.7428571,6.17142857 16.9714286,9.76507937 16.9714286,15.3904762 C16.9714286,21.015873 20.9495108,24.3555556 26.1287671,24.3555556 C30.8688845,24.3555556 35.4669276,21.4349206 35.4669276,15.0603175 C35.4798434,9.84126984 31.8634051,6.17142857 26.4258317,6.17142857 L26.4258317,6.17142857 Z M26.2837573,20.4825397 C23.9330724,20.4825397 22.667319,18.2349206 22.667319,15.2507937 C22.667319,12.6857143 23.6489237,10.0190476 26.2837573,10.0190476 C28.7765166,10.0190476 29.7968689,12.647619 29.7968689,15.2507937 C29.7968689,18.3746032 28.4277886,20.4825397 26.2837573,20.4825397 L26.2837573,20.4825397 Z M68.1052838,15.0984127 C67.8469667,16.0888889 67.562818,17.4095238 67.304501,18.3365079 L67.2011742,18.3365079 C66.9816047,17.4095238 66.6199609,16.1269841 66.3358121,15.0984127 L63.6880626,6.55238095 L57.5659491,6.55238095 L64.0497065,22.4761905 C64.2692759,22.9079365 64.3080235,23.1873016 64.3080235,23.3650794 C64.3080235,23.6190476 64.2046967,23.8984127 64.0238748,24.152381 C63.481409,25.0412698 62.5772994,25.8920635 61.7377691,26.3238095 C60.9111546,26.8190476 60.0328767,27.1365079 59.3483366,27.2507937 L59.3612524,27.2761905 C58.353816,27.5555556 57.6176125,28.4571429 57.6176125,29.5365079 C57.6176125,30.831746 58.6896282,31.8857143 60.007045,31.8857143 C60.1749511,31.8857143 60.3428571,31.8603175 60.5107632,31.8349206 L60.5107632,31.847619 C61.8927593,31.6698413 63.9205479,31.0984127 65.8320939,29.4603175 C67.821135,27.7206349 69.5260274,24.9396825 72.0575342,18.031746 L76.1906067,6.57777778 L70.2880626,6.57777778 L68.1052838,15.0984127 L68.1052838,15.0984127 Z M47.4270059,6.17142857 C41.7440313,6.17142857 37.9726027,9.76507937 37.9726027,15.3904762 C37.9726027,21.015873 41.9506849,24.3555556 47.1299413,24.3555556 C51.8700587,24.3555556 56.4681018,21.4349206 56.4681018,15.0603175 C56.4810176,9.84126984 52.8645793,6.17142857 47.4270059,6.17142857 L47.4270059,6.17142857 Z M47.2849315,20.4825397 C44.9342466,20.4825397 43.6684932,18.2349206 43.6684932,15.2507937 C43.6684932,12.6857143 44.6500978,10.0190476 47.2849315,10.0190476 C49.7776908,10.0190476 50.7980431,12.647619 50.7980431,15.2507937 C50.7980431,18.3746032 49.416047,20.4825397 47.2849315,20.4825397 L47.2849315,20.4825397 Z M14.6982387,4.43174603 C14.7886497,4.44444444 14.8919765,4.45714286 14.9823875,4.45714286 C16.2610568,4.45714286 17.3072407,3.42857143 17.3072407,2.17142857 C17.3072407,1.16825397 16.6485323,0.317460317 15.7444227,0.0126984127 L0.0387475538,0.0126984127 L0.0387475538,23.9746032 L0.142074364,23.9746032 C0.193737769,25.4095238 1.38199609,26.552381 2.85440313,26.552381 C4.32681018,26.552381 5.51506849,25.4095238 5.5667319,23.9746032 L5.5925636,23.9746032 L5.5925636,14.3238095 L13.807045,14.3238095 C13.9232877,14.3365079 14.0395303,14.3492063 14.155773,14.3492063 C15.4086106,14.3492063 16.416047,13.3587302 16.416047,12.1269841 C16.416047,10.9587302 15.5119374,10.0063492 14.3495108,9.91746032 L14.3495108,9.9047619 L14.1816047,9.9047619 L14.155773,9.9047619 L14.1299413,9.9047619 L5.5925636,9.9047619 L5.5925636,4.43174603 L14.6982387,4.43174603 L14.6982387,4.43174603 Z M86.3941292,6.17142857 C80.7111546,6.17142857 76.939726,9.76507937 76.939726,15.3904762 C76.939726,21.015873 80.9178082,24.3555556 86.0970646,24.3555556 C90.837182,24.3555556 95.435225,21.4349206 95.435225,15.0603175 C95.4481409,9.84126984 91.8187867,6.17142857 86.3941292,6.17142857 L86.3941292,6.17142857 Z M86.2391389,20.4825397 C83.888454,20.4825397 82.6227006,18.2349206 82.6227006,15.2507937 C82.6227006,12.6857143 83.6043053,10.0190476 86.2391389,10.0190476 C88.7318982,10.0190476 89.7522505,12.647619 89.7522505,15.2507937 C89.7522505,18.3746032 88.3831703,20.4825397 86.2391389,20.4825397 L86.2391389,20.4825397 Z" id="Shape">
                      </path>
                    </g>
                  </g>
                </svg>
                <p>
                  <span>地址：3 Science Park Drive,The Franklin #02-07,Singapore</span>
                  <span>邮编：118223</span>
                  <span>手机号：(+65)84127233</span>
                  <span>电子邮箱：hi@fooyo.sg</span>
                </p>
              </footer>
            </section>
        </div>
    </div>
  </div>
  <ul id="navBar">
  <li data-menuanchor="firstPage"><a href="#firstPage"></a></li>
    <li data-menuanchor="secondPage"><a href="#secondPage"></a></li>
    <li data-menuanchor="3rdPage"><a href="#3rdPage"></a></li>
    <li data-menuanchor="4thpage"><a href="#4thpage"></a></li>
    <li data-menuanchor="lastPage"><a href="#lastPage"></a></li>
  </ul>
  <script src="<?php bloginfo('template_url'); ?>/js/swiper-3.3.1.min.js"></script>
  <script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/canvasRender.js"></script>
  <script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/fooyo.js"></script>
  <script src='<?php bloginfo('template_url'); ?>/js/app.js'></script>
  <script>
    $(".current-menu-item").addClass('active')

  </script>

</body>
</html>
>>>>>>> origin/master
