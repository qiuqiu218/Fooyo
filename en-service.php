...
<meta property="og:title" content="Experienced Mobile App Development Agency Singapore">
<meta property="og:description" content="Consult the app and website design experts from Fooyo in Singapore and bring the benefits of creativity and innovation to your business.">
<meta property="og:url" content="http://www.fooyo.sg/services/">
<meta property="og:site_name" content="Fooyo|With fooyo, building dreams">
<meta name="twitter:card" content="summary">
<meta name="twitter:description" content="Consult the app and website design experts from Fooyo in Singapore and bring the benefits of creativity and innovation to your business.">
<meta name="twitter:title" content="Experienced Mobile App Development Agency Singapore">
  ...
  <script src="<?php bloginfo('template_url'); ?>/js/fullPage.min.js"></script>
   <script src="<?php bloginfo('template_url'); ?>/js/common.js"></script>
  <link href="<?php bloginfo('template_url'); ?>/css/swiper-3.3.1.min.css" rel="stylesheet">
  <title>Experienced Mobile App Development Agency Singapore</title>
</head>

      <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/en-service.css">
<body>
	<div class="service">
		<div class="top">
		    <h1>
...
<?php get_header();?>
		...
		<div class="banner">
			<h2><em>The Fooyo </em>Services</h2>
			<div>
				<p>Fooyo specializes in creating clean,	modern and intuitive products to help scale	new products and ideas.</p>
				<p>We believe great	products are made iteratively. We adopt	the	LEAN startup approach in	our	new	product	innovation processes,</p>
				<p>all the way from the idea stage to the design and development stage.</p>
			
			</div>
		</div>
		<ul class="tec clearfix">
			
			<?php query_posts('cat=134'); while(have_posts()): the_post(); ?>   
  						<li>
		...
		<div class="content">
			...
			<p>We enjoy the process to work closely with our clients and partners to build products from a scratch. </p>
			<p>We feel excited seeing products taking off and loved by thousands of users.</p>
			<div class="swiper-container">
				...
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<ul class="clearfix">
							<?php query_posts('cat=136'); while(have_posts()): the_post(); ?>   
  							<li>
	  					<?php   the_post_thumbnail( 'full' );  ?>
	  					<p><?php the_title();?></p>
	  					</li>
						...
						<?php query_posts('cat=150'); while(have_posts()): the_post(); ?> 
							<small><?php the_content();?></small>
							<?php endwhile; wp_reset_query(); ?>
					</div>
					<div class="swiper-slide">
						<ul class="clearfix">
						<?php query_posts('cat=142'); while(have_posts()): the_post(); ?>   
  							<li>
	  					<?php   the_post_thumbnail( 'full' );  ?>
	  					<p><?php the_title();?></p>
	  					</li>
						...
						<?php query_posts('cat=146'); while(have_posts()): the_post(); ?> 
							<small><?php the_content();?></small>
										<?php endwhile; wp_reset_query(); ?>
					</div>
					<div class="swiper-slide">
						<ul class="clearfix">
							<?php query_posts('cat=144'); while(have_posts()): the_post(); ?>   
  							<li>
	  					<?php   the_post_thumbnail( 'full' );  ?>
	  					<p><?php the_title();?></p>
	  					</li>
						<?php endwhile; wp_reset_query(); ?>
						</ul><?php query_posts('cat=148'); while(have_posts()): the_post(); ?> 
							<small><?php the_content();?></small>
							<?php endwhile; wp_reset_query(); ?>
					</div>
				</div>
    	...
    	<div class="service-contain">
    		<?php query_posts('p=612'); while(have_posts()): the_post(); ?> 
    						<h1><?php the_title();?></h1>
    						<div class="service-contain-inner">
							<p><?php the_content();?></p>
							</div>
							<?php endwhile; wp_reset_query(); ?>
    	</div>
	...
	<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/swiper-3.3.1.min.js"></script>
	<script type="text/javascript">
		!(function($){
			var swiper = new Swiper('.service .content .swiper-container',{
				prevButton:'.swiper-button-prev',
				nextButton:'.swiper-button-next',
			})
			var $lis  = $('.swiper-container > ul li');
			$lis.on('click',function(e){
				var index = $(this).index();
				swiper.slideTo(index);
				tab($lis,index,'active');
			})
			function tab(doms,index,className){
				$(doms).eq(index).addClass(className).siblings().removeClass(className);
			}
			$lis.eq(0).addClass('active');
			$('.swiper-button-next').on('click',function(e){
				tab($lis,swiper.snapIndex,'active');
			})
